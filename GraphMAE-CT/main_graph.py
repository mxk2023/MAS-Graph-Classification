...
from gptrans_dataset.data import load_graph_classification_dataset
...
def main(args):
    ...
    num_layers_enc_trans = args.num_layers_enc_transformer
    ...
    graphs, (num_features, num_classes,n_edge_features) = load_graph_classification_dataset(dataset_name,args.random_walk_length,aug_ratio,device=device,deg4feat=deg4feat)
    args.n_edge_features = n_edge_features
    ...
    for i, seed in enumerate(seeds):
        ...
        if logs:
            logger = TBLogger(name=f"{dataset_name}_loss_{loss_fn}_rpr_{replace_rate}_nh_{num_hidden}_nlgnn_{num_layers_enc_gnn}_nltrans_{num_layers_enc_trans}_lr_{lr}_mp_{max_epoch}_mpf_{max_epoch_f}_wd_{weight_decay}_wdf_{weight_decay_f}_{encoder_type}")
        else:
            logger = None
