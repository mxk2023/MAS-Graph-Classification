...
class PreModel(nn.Module):
    ...
    def mask_attr_prediction(self,batched_data ):
        ...
        # x2,edge_index2,pe2,in_degree2,out_degree2 = batched_data.x2,batched_data.edge_index2,batched_data.pe2,batched_data.in_degree2,batched_data.out_degree2
        # batched_data.x = x2
        # batched_data.edge_index = edge_index2
        # batched_data.edge_attr = edge_attr2
        # add_pe=AddRandomWalkPE(40)
        # aug_pe=add_pe(batched_data).random_walk_pe

        # adj2 = torch.zeros([x2.size(0), x2.size(0)], dtype=torch.bool).cuda()
        # adj2[edge_index2[0, :], edge_index2[1, :]] = True
        # in_degree2 = adj2.long().sum(dim=1).view(-1)
        # out_degree2 = adj2.long().sum(dim=0).view(-1)
        ...
        # padding mask tokens
        padding_h = torch.zeros([use_x.size(0),h_aug.size(1)]).cuda(x.device)
        padding_h[keep_nodes] = h_aug[keep_nodes]

        t_rep = torch.cat((self.dec_node_norm(padding_h),pe),1)
        t_rep = self.dec_pe_encoder(t_rep)        
