...
from model_ppa.gnn import GCN,SAGE
    ...
    def __init__(self,gnn_type,gate_type,input_size,hidden_size,out_size,num_experts,min_layers, noisy_gating=True, k=4 , coef=1e-3,dropout=0.5,gate_dropout=0.2,heads=4):
        ...
        if gnn_type == 'GCN':     
            for i in range(min_layers,num_experts+min_layers):
                self.experts.append(GCN(input_size,hidden_size,out_size,i,dropout=dropout))
        elif gnn_type == 'SAGE':
            for i in range(min_layers,num_experts+min_layers):
                self.experts.append(SAGE(input_size,hidden_size,out_size,i,dropout=dropout))         
        ...
        if gate_type == 'liner':
            ...
            self.gate_model=GCN(input_size,hidden_size,num_experts,2,dropout)
        elif gate_type =='SAGE':
            self.gate_model=SAGE(input_size,hidden_size,num_experts,2,dropout)
